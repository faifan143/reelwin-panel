...
export default function AdminPage({ onLogout }: AdminPageProps) {
  ...
  const onSubmit = async (data: ContentFormData) => {
    ...
    // Handle interest IDs
    if (typeof data.interestIds === 'string') {
      // If interestIds is provided as comma-separated string
      const idsArray = data.interestIds.split(',').map(id => id.trim()).filter(id => id);
      idsArray.forEach(id => formData.append("interestIds[]", id));
    } else if (Array.isArray(data.interestIds)) {
      // If interestIds is already an array
    ...
    try {
      ...
      setShowError(true);
      setErrorMessage(error.response?.data?.message || "حدث خطأ أثناء إنشاء المحتوى");
    } finally {
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-4">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-white">لوحة إدارة ReelWin</h2>
            <button
        ...
        <div className="p-6">
          <div className="mb-6">
            <h3 className="text-xl font-bold text-gray-800 border-b pb-2">إضافة محتوى جديد</h3>
            <p className="text-gray-600 mt-2">أدخل تفاصيل المحتوى والوسائط المطلوبة</p>
          </div>

          <form ref={formRef} onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            {/* Basic Information */}
            <div className="bg-gray-50 p-4 rounded-lg">
              <h4 className="font-semibold text-gray-700 mb-3">المعلومات الأساسية</h4>
            ...
            {/* Owner Information */}
            <div className="bg-gray-50 p-4 rounded-lg">
              <h4 className="font-semibold text-gray-700 mb-3">معلومات المالك</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  <label htmlFor="ownerNumber" className="block font-medium text-gray-700 mb-1">
                    رقم المالك *
                  </label>
                  <input
                    id="ownerNumber"
            ...
            {/* Content Settings */}
            <div className="bg-gray-50 p-4 rounded-lg">
              <h4 className="font-semibold text-gray-700 mb-3">إعدادات المحتوى</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="intervalHours" className="block font-medium text-gray-700 mb-1">
                    ساعات الفاصل *
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  <label htmlFor="endValidationDate" className="block font-medium text-gray-700 mb-1">
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <input
                        ...
                        className={`w-full px-4 py-2 border ${
                          errors.endValidationDate ? "border-red-500" : "border-gray-300"
                        } rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500`}
                        {...field}
                      />
                    )}
                ...
                <div className="md:col-span-2">
                  <label htmlFor="interestIds" className="block font-medium text-gray-700 mb-1">
                    معرفات الاهتمامات (اختياري)
                  </label>
                  <input
                    id="interestIds"
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="أدخل معرفات الاهتمامات مفصولة بفواصل"
                    {...register("interestIds")}
                  />
                  <p className="text-gray-500 text-xs mt-1">
                    أدخل معرفات الاهتمامات مفصولة بفواصل (مثال: id1,id2,id3)
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-gray-500 text-sm mr-3">
                    {imageFiles.length > 0 ? `تم اختيار ${imageFiles.length} صورة` : "لم يتم اختيار أي صورة"}
                  </span>
                </div>
              ...
              <div>
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-gray-500 text-sm mr-3">
                    {videoFiles.length > 0 ? `تم اختيار ${videoFiles.length} فيديو` : "لم يتم اختيار أي فيديو"}
                  </span>
                </div>
