...
// Gems APIs
export const generateGem = async (
  data: GenerateGemDto
): Promise<GemResponse> => {
...
export const selectRandomWinner = async (
  id: string
): Promise<{ winner: any }> => {
...
// Bulk once QR: generate printable PDF
export const generateBulkOncePdf = async (
  payload: BulkOncePdfDto
): Promise<Blob> => {
  const response = await api.post("/qr/bulk-once/pdf", payload, {
    responseType: "blob",
  });
  return response.data;
};
